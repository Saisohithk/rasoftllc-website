  ...
  template: `
    <section class="py-24 px-[5vw] bg-[#0b0e18]">
      <div class="max-w-7xl mx-auto">
        <div class="flex items-center gap-3 text-[#E8280B] text-xs font-bold tracking-[2.5px] uppercase mb-4"><span class="w-6 h-px bg-[#E8280B]"></span>Client Stories</div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-5">
          @for (t of testimonials; track t.name) {
            <div class="bg-[#0f1420] border border-white/[0.08] p-8 relative">
              ...
              <div class="flex mb-3">@for (s of [1,2,3,4,5]; track s){<span class="text-[#FFAB00] text-sm">★</span>}</div>
              ...
              <div class="flex items-center gap-3 border-t border-white/[0.06] pt-5">
                <div class="w-10 h-10 bg-[#E8280B] flex items-center justify-center font-heading text-sm text-white flex-shrink-0" style="clip-path:polygon(0 0,85% 0,100% 15%,100% 100%,15% 100%,0 85%)">{{t.initials}}</div>
                <div><div class="text-sm font-semibold text-white">{{t.name}}</div><div class="text-[11px] text-white/35 tracking-wide">{{t.role}}</div></div>
              </div>
            </div>
          }
        </div>
